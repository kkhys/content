---
title: Claude CodeのSubagentsで記事の校正を効率化する
description: Claude CodeのSubagents機能を活用したブログ記事校正の実践例である。構造分析、言語編集、読みやすさ向上、技術検証の4つの専門エージェントが並列でレビューを行い、記事品質を効率的に改善する手法を紹介している。
emoji: ✳
category: Tech
tags: [AI]
status: published
...
つい最近、Claude Codeに [Subagents](https://docs.anthropic.com/en/docs/claude-code/sub-agents) という機能が追加された。
早速、試しに使ってみたところ、これがブログの校正作業と相性が良かったので、そのあたりの顛末を記しておこうと思う。

## Subagentsとは

Subagentsとは、Claude Codeがタスクを委任できる、事前設定されたAIパーソナリティのことである。
つまり、各分野ごとに専門家を用意して、依頼したタスクがその専門家の領域であれば委任するという流れを組むことができる。

まとめると、Subagentsの特徴としては以下の4点があげられる。
...
ただし、Custom slash commandsはメインスレッド上で実行されてしまうため、複雑な処理がCustom slash commandsに含まれていると、あっという間にauto-compactされてしまい、出力精度が悪くなってしまう。
通常のやり取りも同様である。

SubagentsではCustom slash commandsで実行させる作業を移譲することで、大幅なコンテキストの節約ができる（Subagentsで実行されるタスクはメインスレッドとは異なるコンテキストウィンドウを持つため）。
例えば、 品質チェックを行うCustom slash commandsを実行させているのであれば、Subagentsに品質チェックを行うレビュワーとして定義するという具合に。

さらに、複数のSubagentを同時に動かすことで、並列作業も可能になる。
人間でいえば、複数の部署に同時に指示を出して、それぞれが独立して作業を進めるような状況である。
そのメリットを活かすためにも各Subagentsは [Small, Focused Agents](https://github.com/humanlayer/12-factor-agents/blob/main/content/factor-10-small-focused-agents.md) として構築した方が良い。
...
Claude Code上で`/agents`コマンドを実行すると対話形式でSubagentsを作成できる。
最初はCLIで雛形を作成してもらい、後から手動で変更したり、Claude Codeに編集してもらうのがおすすめのやり方。

## ブログ記事の校正を行う編集者を作成

当ブログはMarkdown形式[^1] で記事を管理している（[kkhys/content](https://github.com/kkhys/content)）ため、ローカル環境でClaude Codeを使った校正が可能。
今まではCustom slash commandsで誤字脱字や修正点がないかを検証していたが、Subagentsを使った方法に切り替える。

[^1]: 正確にはMDX。Astroでコンポーネントの置き換えを行うには拡張子を`.mdx`にしないとダメだった。本当はPlaneなMarkdownにしたい

先ほど記載したように`/agents`コマンドを実行して、各質問に答えると`.claude/agents`以下にSubagentsファイルが作成される。
次に作成されたファイルの中身を見て、分解できないか検証する。
今回のケースでは以下のような役割ごとにファイルを分割した（役割の分割とファイル生成もClaude Codeで実行）。
...
Claude Codeはdescriptionを見て、動的にSubagentsを選択してくれるので、あとはそのままプロンプトを書けば良い。
と言いたいところだが、「この記事のレビューを行なってください」と依頼してもSubagentsが使われないこともあった。
...
```shell
> @content/blog/2025-07-21/index.mdxこの記事のレビューを行なってください
```
...
⏺ 📝 Comprehensive Content Review Results
  ...
  具体的修正例:
  現在：AIが考えを巡らせている間、人間の方はただ待つしかない。
  改善：AIが処理を実行している間、人間はただ待機するしかありません。
...
この記事も含めて、普段はWebStormで記事を書いている。
記事を書き終わったタイミングでターミナルを開いて、Claude Codeにレビューを依頼。
そして、レビューの修正が終われば`git commit & push`で記事の公開がIDE上で完結する流れは、Subagentsを取り入れることで、より強固になったと感じる。

今回はブログ記事のレビューに特化したSubagentsを作成した。
