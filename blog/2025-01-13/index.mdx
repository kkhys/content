---
title: "v2.1.0リリースノート: タグクラウドの追加"
description: v2.1.0をリリースし、タグページとタグクラウド機能を追加した。SSGで実装することで表示速度と安定性を向上させ、絵文字のSVG化やContentlayerを活用した効率的な開発を実現している。
emoji: 🚀
category: Tech
tags: ["Release note"]
status: published
...
このブログの記事は [Contentlayer](https://contentlayer.dev)（正確にはフォーク版の [Contentlayer2](https://github.com/timlrx/contentlayer2)）を使い、MDX形式で書かれている。
...
ここで少し背景の話をしておこう。
Contentlayerでは、すべての記事データを1つのオブジェクトにまとめてくれる便利機能がある（たとえばpostであれば`allPosts`というオブジェクトに）。
...
絵文字をSVG化する際は [Satori](https://github.com/vercel/satori) というライブラリを使用している。
[以前の記事](/blog/posts/b15e6x7) ではSatoriを使って同様に絵文字（Noto Emoji）をSVGに変換していたが、今回使用するのはカラーフォントなので、前回のように簡単にはいかなかった。
...
今回はMicrosoftの [Fluent Emoji](https://fluentemoji.com) という絵文字に変換しているが、他にも以下の絵文字に変換することも可能。
