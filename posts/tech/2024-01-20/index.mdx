...
description: ブログを公開してまだ2記事目だけど、早くもバージョン1.4.0をリリースした。このサイトはある意味、実験場のような場所なので、妥協はせずに面白い機能を追加していく予定である。
emoji: 🚀
category: Tech
tags: [Release]
status: published
...
```text
https://kkhys.me/posts/649ip2KUn

↓

https://kkhys.me/posts/649ip2KUn#見出しにアンカーリンクを追加
```
...
```typescript title="Currying sample by TypeScript"
const add = (x: number, y: number): number => x + y;
const curriedAdd = (x: number): (y: number) => number => (y: number) => add(x, y);
...
ちなみに [1 つ前の記事](/posts/BAs2bVTaP) に投稿した「海辺にいた白猫」の Blur 画像は 以下のようになる。
